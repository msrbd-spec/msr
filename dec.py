...
Usage:
    python dec.py
    python dec.py --input calibration.json external_validation.json fedper.json gradcam_reliability.json
    python dec.py --input-dir ./jsons --output ./outputs
"""
...
def main():
    ...
    parser.add_argument(
        "--output", "-o",
        default="./outputs",
        help="Output directory for restored PNG files (default: ./outputs).",
    )
    ...
    if args.input_dir:
        ...
        # Default: look for the 4 known JSON files in current directory
        defaults = [
            "calibration.json",
            "external_validation.json",
            "fedper.json",
            "gradcam_reliability.json",
        ]
        for jf in defaults:
