...
Reads a JSON file where keys are relative file paths and values are
base64-encoded PNG binary data, then writes each PNG to its original path.

Usage:
    python dec.py
    python dec.py --input figs_archive.json --output ./outputs/figs
"""
...
def main():
    ...
    parser.add_argument(
        "--input", "-i",
        default="figs_archive.json",
        help="Input JSON archive file (default: figs_archive.json).",
    )
    parser.add_argument(
        "--output", "-o",
        default="outputs/figs",
        help="Output directory for restored PNG files (default: outputs/figs).",
    )
