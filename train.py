...
if __name__ == '__main__':
    ...
    # Single-GPU mode — A100/H100 have 40-80GB, batch size 32 is comfortable.
    BATCH_SIZE  = 32
    NUM_WORKERS = 8
    ...
    def _run_parallel_clients(build_fn, fold_dir, run_dir_name, global_state_cpu,
        ...
        # Build a temporary global model to pass into _fedprox_local_update
        global_model = build_fn()
        unwrap(global_model).load_state_dict(global_state_cpu)
        global_model = global_model.to(DEVICE)

        # Create focal loss once (same alpha/gamma for all clients in a round)
        focal_loss = FocalLoss(alpha=focal_alpha, gamma=focal_gamma)

        # Build client dataloaders (same logic as train_fedprox)
        tr_loaders = []
        for c in range(1, NUM_CLIENTS + 1):
        ...
        for ci in range(NUM_CLIENTS):
            ...
            # Convert model object -> state_dict (CPU) for aggregation functions
...
if __name__ == '__main__':
    ...
    def run_one(fold, run_name, fold_dir, base_save):
        ...
        _load_swa_state(swa_c_model, swa_c)
        ...
        _load_swa_state(swa_fl_model, swa_fl)
