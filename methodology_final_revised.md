...
\begin{figure}[htbp]
    ...
    \vspace{5cm}
    \caption{Overall framework showing the federated learning setup with five clients and the central server. The left panel depicts the data partitioning scheme (both IID Uniform and Non-IID Heterogeneous settings), the middle panel shows the client-side local training pipeline (PoxCSAF-Net with FocalLoss and FedProx proximal regularization), and the right panel illustrates the server aggregation step and global model evaluation loop.}
...
\end{equation}
where $\ell(\cdot)$ is the task-specific loss function and $f(\cdot;\mathbf{w})$ is PoxCSAF-Net parameterized by $\mathbf{w}$.
...
\caption{Centralized Training of PoxCSAF-Net with Phased Unfreezing and SAM}
...
\begin{algorithm}[htbp]
\caption{FedProx Federated Training of PoxCSAF-Net}
\label{alg:fedprox}
\begin{algorithmic}[1]
...
Two adaptations are required to make PoxCSAF-Net compatible with Opacus:
...
\begin{table}[htbp]
    \centering
    \caption{Hyperparameters and implementation details for the proposed PoxCSAF-Net model.}
